...
class TrainDataLoader(object):
    ...
    def _pad_crop_resize_detection(self):
        ...
        # p = round((tw + th) / 2, 2)  # 生成边长为目标长宽均值的正方形框，对应于标签的anchor
        # template_square_size = int(np.sqrt((tw + p) * (th + p)))  # a   根据目标大小生成模板
        # detection_square_size = int(template_square_size * 2)  # A =2a    detection的大小
        #
        # # pad
        # detection_lt_x, detection_lt_y = cx - detection_square_size // 2, cy - detection_square_size // 2  # 目标中心减去detection输入大小
        ...
        # new_w, new_h = left + right + w, top + bottom + h  # template所在位置与加上大小后，可能会超过图像边界，需要补pad，以保证固定输入
        #
        # # pad load
